    ...
    extends Storage
{
    ...
    {
        /*
         хочу для каждого свойства моего класса создать временную переменную
         **/
        foreach ($this as $k => $v) {
            $$k = $v; // переменная с именем, которое содержится в переменной к, можно писать ${$k}
            /*
             * для каждого свойства
             * создастся временная переменная
...
/*
 * Вопрос: а если я не знаю какие переменные доступны в шаблоне. Как просмотреть все доступные?
 * Ответ: В php есть специальная функция, которая показывает список доступных в данный момент переменных.
 * */

/*
 * Как обычно делается в сложных системах, когда шаблоны и контроллеры пишут разные люди:
 * специальный синтаксис комментария phpdoc
 * появляются подсказки
 * и вообще хороший тон,
 * разработчик шаблона сразу видит какие переменные есть
 * в шаблоне пишется
